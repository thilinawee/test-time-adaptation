...
def get_accuracy(model: torch.nn.Module,
    ...
    AVG_ACC_LOG_FREQ = cfg.AVG_ACC_LOG_FREQ if cfg.AVG_ACC_LOG_FREQ != -1 else print_every
    ...
    with torch.no_grad():
        for i, data in enumerate(train_data_loader):
            ...
            if print_every > 0 and (i+1) % print_every == 0:
                ...
                # logging various variables important for analysis
                if cfg.LOG_PRIOR:
                    prior_log = ", ".join([f"class_{j}: {prob.item():.4f}" for j, prob in enumerate(model._prior)])
                    logger.info(f"Prior probabilities for {domain_name} at step {i+1}: {prior_log}")

                if cfg.LOG_LOSS:
                    logger.info(f"Training loss for {domain_name} at step {i+1}: {model.loss.item():.6f}")

            if print_every > 0 and (i+1) % (AVG_ACC_LOG_FREQ) == 0:
                ...
                if cfg.LOG_AVG_ACC:
                    ...
                    logger.info(f"AllAcc for {domain_name} at step {i+1}: {avg_acc:.4f}")
    ...
    logger.info(f"Average AllAcc: {all_cls_avg_acc:.2%}")
