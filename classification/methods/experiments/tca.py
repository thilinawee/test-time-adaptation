...
class TCA(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        # TCA specific hyperparameters
        self.lambda_align = getattr(cfg, 'LAMBDA_ALIGN', 0.025)  # λ1 in paper
        self.lambda_topology = getattr(cfg, 'LAMBDA_TOPOLOGY', 0.15)  # λ2 in paper
        self.alpha = getattr(cfg, 'ALPHA', 0.1)  # EMA coefficient for centroids
        self.beta = getattr(cfg, 'BETA', 0.9)   # EMA coefficient for topology weights
        self.t_param = getattr(cfg, 'T_PARAM', 2.0)  # Temperature parameter
        self.confidence_threshold = getattr(cfg, 'CONFIDENCE_THRESHOLD', 0.72)
        self.teacher_momentum = getattr(cfg, 'TEACHER_MOMENTUM', 0.999)  # Momentum for EMA teacher model

        # CoTTA related parameters (keeping for compatibility)
        self.mt = getattr(cfg.M_TEACHER, 'MOMENTUM', 0.999) if hasattr(cfg, 'M_TEACHER') else 0.999
        self.rst = getattr(cfg.COTTA, 'RST', 0.01) if hasattr(cfg, 'COTTA') else 0.01
        self.ap = getattr(cfg.COTTA, 'AP', 0.72) if hasattr(cfg, 'COTTA') else 0.72
        self.n_augmentations = getattr(cfg.TEST, 'N_AUGMENTATIONS', 32) if hasattr(cfg, 'TEST') else 32
        ...
        # Setup anchor model (keeps original parameters)
        ...
        arch_name = "resnet50"  # Adjust based on your model
        dataset_name = "imagenet_c"  # Adjust based on your dataset
        self.feature_extractor, self.classifier = split_up_model(self.model, arch_name, dataset_name)
        
        # Initialize TCA components
        self.class_centroids = {}  # Store centroids for each class
        self.prev_topology_weights = None  # Previous batch topology weights
    ...
    def loss_calculation(self, batch):
        ...
        # total_loss = sce_loss + self.lambda_topology * topology_loss
    ...
    def reset(self) -> None:
        ...
        self.anchor_model = self.model

        print(f"Model device: {next(self.model.parameters()).device}")
        print(f"Teacher model device: {next(self.teacher_model.parameters()).device}")
        print(f"Anchor model device: {next(self.anchor_model.parameters()).device}")
