...
def confidence_diff_mask(original_model, adapted_model, x):
...
class SAR_LA_APX(TTAMethod):
    ...
    def get_forward_function(self, cfg):
        ...
        if cfg.LOGIT_ADJUST.TYPE == 'softmax_prior_la':
            return self.forward_and_adapt_softmax_prior
        elif cfg.LOGIT_ADJUST.TYPE == 'prediction_prior':
            return self.forward_and_adapt_prediction_prior
        elif cfg.LOGIT_ADJUST.TYPE == 'prediction_prior_sqrt_la':
            return self.forward_and_adapt_prediction_prior_sqrt
        elif cfg.LOGIT_ADJUST.TYPE == 'confidence_diff_prediction_prior_sqrt_la':
            return self.forward_and_adapt_confidence_diff_prediction_prior_sqrt_la
        else:
            raise ValueError(f"Unsupported logit adjustment type: {cfg.LOGIT_ADJUST.TYPE}")

    @torch.enable_grad()  # ensure grads in possible no grad context for testing
    def forward_and_adapt_confidence_diff_prediction_prior_sqrt_la(self, x):
        ...
        confidence_mask = confidence_diff_mask(self.original_model, self.model, imgs_test)
        ...
        with torch.no_grad():
            ...
            class_counts = torch.bincount(predictions[confidence_mask], minlength=outputs.shape[1]).float().clamp(min=self.EPSILON)
        ...
        confidence_filtered_outputs = outputs[confidence_mask]

        # filtering reliable samples/gradients for further adaptation; first time forward
        entropys = self.softmax_entropy(confidence_filtered_outputs)
        ...
        second_output = self.model(imgs_test) + self.TAU *  torch.log(self._prior.clone()) if self.TAU > 0 else self.model(imgs_test)
        second_confidence_filtered_outputs = second_output[confidence_mask]

        entropys2 = self.softmax_entropy(second_confidence_filtered_outputs)
    ...
    @torch.enable_grad()  # ensure grads in possible no grad context for testing
    def forward_and_adapt_prediction_prior_sqrt(self, x):
        """Forward and adapt model input data.
        Measure entropy of the model prediction, take gradients, and update params.
        """
        imgs_test = x[0]
    ...
    @torch.enable_grad()  # ensure grads in possible no grad context for testing
    def forward_and_adapt_softmax_prior(self, x):
        """Forward and adapt model input data.
        Measure entropy of the model prediction, take gradients, and update params.
        """
        imgs_test = x[0]
